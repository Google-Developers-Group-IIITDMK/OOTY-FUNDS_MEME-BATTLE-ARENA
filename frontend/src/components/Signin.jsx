...
export default function MemeBattleSignIn() {
  ...
  // ENV variables (Vite). If you're using CRA replace with process.env.REACT_APP_...
  ...
  const signIn = async (e) => {
    ...
    setLoading(true);
    try {
      // Example payload — adapt to your backend contract
      ...
      const res = await axios.post(`${API_URL}/auth/signin`, payload, { headers, timeout: 12000 });

      // handle response based on your API shape
      if (res?.data?.token) {
        setSuccess("Signed in successfully — welcome to Meme Battle Arena!");
        // Save token securely (example uses localStorage) — adapt as needed
